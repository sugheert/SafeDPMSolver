/* ══════════════════════════════════════════════════════════════════════
   SafeDPMSolver — CFG+CBF Three-Panel Visualiser
   Plain CFG DPM (left)  |  CFG+CBF Before Control (middle, interactive)
   CFG+CBF After Control (right, passive)  |  Inspector
   ══════════════════════════════════════════════════════════════════════ */
...
const DEFAULTS_KEY = 'safedpm_cfg_defaults';
...
function loadDefaults() {
  ...
  try {
    ...
    setEl('gs-slider', params.guidance_scale);
    setEl('gs-val', Number(params.guidance_scale).toFixed(1));
...
function bindParamControls() {
  ...
  // guidance_scale slider — updates params but does NOT trigger recompute
  // (it only takes effect on the next Run; CFG is baked into cached eps_pred)
  document.getElementById('gs-slider').addEventListener('input', () => {
    params.guidance_scale = parseFloat(document.getElementById('gs-slider').value);
    document.getElementById('gs-val').textContent = params.guidance_scale.toFixed(1);
  });
