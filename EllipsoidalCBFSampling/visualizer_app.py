...
from EllipsoidalCBFSampling.models.samplers_ellipses import dpm_solver_1_sample, dpm_solver_1_cbf_sample, recompute_cbf_step
from EllipsoidalCBFSampling.CBF.trajectory_cbf_ellipses import compute_cbf_metrics

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------
CHECKPOINTS_DIR = PROJECT_DIR.parent / 'checkpoints'
...
def run_optimisation(req: RunRequest):
    ...
    # Build obstacle tensor [N, 4]
    obs_tensor = [[obs.x, obs.y, obs.a, obs.b] for obs in req.obstacles]
    obstacles = torch.tensor(
        obs_tensor, dtype=torch.float32, device=DEVICE
    )
    ...
    try:
        ...
        t0 = time.perf_counter()
        _, safe_history, before_history, cbf_history = dpm_solver_1_cbf_sample(
            ema_model, ve,
            x_start=x_start, x_goal=x_goal,
            obstacles=obstacles,
            T_steps=T_steps, n_steps=req.n_steps,
...
def batch_run(req: BatchRunRequest):
    ...
    obs_tensor = [[obs.x, obs.y, obs.a, obs.b] for obs in req.obstacles]
    obstacles = torch.tensor(obs_tensor, dtype=torch.float32, device=DEVICE)
